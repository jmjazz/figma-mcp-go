...
`mcp__figma-mcp-go__get_variable_defs` 또는 `mcp__figma-mcp-go__whoami` 등 figma-mcp-go 도구를 호출해 현재 연결 상태를 확인합니다.
...
Cursor / VS Code는 `.vscode/mcp.json`에 동일하게 추가합니다.

### B. 플러그인 최초 import를 아직 안 한 경우 (최초 1회)

1. **Figma 데스크톱 앱** 실행 (브라우저 버전 X)
2. [GitHub Releases](https://github.com/vkhanhqui/figma-mcp-go/releases)에서 `plugin.zip` 다운로드 후 압축 해제
...
1. **변수가 있는 Figma 파일** 열기
   - ⚠️ 변수가 별도 DS 라이브러리 파일에 있으면 **그 라이브러리 파일**을 열어야 합니다
2. **Plugins → Development → Figma MCP Go** 실행
3. 플러그인 창을 **열어둔 채**로 유지 (닫으면 브릿지 끊김)
